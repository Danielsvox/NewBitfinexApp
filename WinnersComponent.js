...
const WinnersComponent = ({ navigation }) => {
    const { tickers, getLogoFilename } = useContext(TickersContext);
    const Winners = [...tickers].sort((a, b) => b.tickerData.daily_change_relative - a.tickerData.daily_change_relative).slice(0, 10);

    return (

        <ScrollView style={styles.scrollView}>
            {Winners.map((ticker, index) => (
                <TouchableOpacity
                    key={index}
                    onPress={() => {
